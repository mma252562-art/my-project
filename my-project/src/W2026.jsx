...
export default function Wel() {
  ...
  return (
    <div>
      <div className="  bg-[#131722]  w-full h-screen flex flex-col items-center gap-10 pt-20">
        <div className='grid grid-cols-6 gap-4 flex-wrap '>
        <h1 className='text-white col-span-2 font-bold text-5xl'>TRENDING MOVIES </h1>
          {TrindingMov.slice(0, 10).map((TrindingMov) => (
            <div key={TrindingMov.title} className=" ">
              <img src={TrindingMov.thumbnail} alt={TrindingMov.title} className="w-80 h-100 rounded-4xl" />
              <p className=' text-center text-white'>{TrindingMov.title}</p>
            </div>
          ))}
        </div>
      </div>
